...
async function getData(){
    var data = await fetch("https://api.odcloud.kr/api/15031992/v1/uddi:3c46e4ef-78dc-4da6-97b1-ad226624eff5_201911131644?page=1&perPage=500&serviceKey=ymqZ2e7%2FnpnUWYCS9B9Zv65We7NUOtTOzGLwPyNlFnegW4cGrvvzxIOnHaleT%2F3JTcgci5x%2FcdQcpdCkBVBgwA%3D%3D").then(function(res){ return res.json();});
    return data.data;
}
...
function search(){
    ...
    $(".item_short").filter(function(){
        ...
        if(year.length!=0 && isShow){
            if(Number(data_list[idx].설치년도) >= Number(year[0]) &&
                 Number(data_list[idx].설치년도) < Number(year[0])+100) isShow=true;
            else isShow = false;
        }
